...
RUN NO_DOCKER=1 make build && \
    mkdir -p /tmp/build && \
    cp /go/src/github.com/openshift/cluster-node-tuning-operator/bin/cluster-node-tuning-operator-tests-ext /tmp/build/cluster-node-tuning-operator-tests-ext && \
    gzip /tmp/build/cluster-node-tuning-tests-ext
...
COPY --from=builder /tmp/build/cluster-node-tuning-operator-tests-ext.gz .
